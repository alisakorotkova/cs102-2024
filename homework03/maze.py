...
def solve_maze(
    ...
    :return:
    """
    # 1. проверяем, что выходов больше одного (помним, мы можем задавать их случайным образом).
    # Если это не правда - путь найден, это координаты входа (она же координата выхода).
    # Возвращаем лабиринт и координаты
    ...
    while grid[out_[0]][out_[1]] == 0:
        k += 1
        make_step(grid, k)
    path = shortest_path(grid, out_)
    return grid, path
